...
function WorkflowView({
  ...
  const nodes = useMemo(() => (preview && Array.isArray(preview.nodes)
  ...
  async function generate() {
    ...
    setMessage("");
    try {
      const next = await api.workflowPlanPreview(prompt.trim(), { source: "fleet-ide" });
      setPreview(next);
    } catch (error) {
  ...
  return (
    <main className="workbench-view">
      <ViewHeader description="Compile an objective into a draft graph, then review it before anything is created." eyebrow="Workflow planning" title="Workflow studio" />

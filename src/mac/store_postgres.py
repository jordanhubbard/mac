...
class PostgresStore(StoreHelpersMixin):
    ...
    def initialize(self) -> None:
        ...
        # bundled schema. These upgrade a database that already carries an
        # earlier, partial version of it -- the `reviews.findings` failure mode,
        # where a column added to schema.sql never reaches a database created
        # before it existed because CREATE TABLE IF NOT EXISTS is a no-op on a
        # table that is already there. Only the outcome columns are listed:
        # epoch_id, agent_id and generation are the table's identity and are
        # referenced by the index in schema.sql, which runs before this point,
        # so a database missing one of those is not something an ADD COLUMN can
        # rescue. The NOT NULL columns carry a DEFAULT because the existing
        # table may hold rows; the CHECK on terminal_state only ever reaches

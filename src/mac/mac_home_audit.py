...
* **Strictly read-only.**  Nothing here creates, writes, chmods or deletes.
  The only filesystem calls are ``iterdir``, ``exists``, ``is_dir``,
  ``is_file``, ``is_symlink`` and ``readlink`` — every one a stat or a
  directory read.  ``tests/test_mac_home_audit.py`` snapshots the fixture
  tree's listing, sizes and mtimes around an audit and asserts they are
  unchanged.
...
# No __all__: scripts/generate-env-config-registry.py harvests every MAC_*
# token out of this module's string constants, so listing the schema constant's
# name in one would publish it to docs/env-config-reference.md as an
# environment variable that does not exist.  The public surface is the
# undecorated names below (everything not prefixed with an underscore).
...
def _build_container_buckets() -> Dict[str, Optional[str]]:
    """Canonical bucket owning each container, including at legacy positions.

    A container's children are attributed to the §4 bucket that owns the
    container, not to the directory they happen to sit in today: the gateway
    home's children read as ``gateway`` whether they are found under
    ``gateway/openclaw`` or at the pre-migration ``openclaw`` root position.
    """
    owners: Dict[str, Optional[str]] = {bucket.name: bucket.name for bucket in CANONICAL_BUCKETS}
    for bucket in CANONICAL_BUCKETS:
        for entry in bucket.entries:
            if entry.descend:
                owners[f"{bucket.name}/{entry.name}"] = bucket.name
    for legacy in LEGACY_ROOT_ENTRIES:
        if legacy.canonical_target and legacy.canonical_target in owners:
            owners[legacy.name] = owners[legacy.canonical_target]
    return owners


_CONTAINERS: Dict[str, Tuple[ExpectedEntry, ...]] = _build_containers()
_CONTAINER_BUCKETS: Dict[str, Optional[str]] = _build_container_buckets()
...
def _classify_child(
    ...
    record: Dict[str, Any] = {
        ...
        "classification": CLASSIFICATION_DRIFT,
        "layout_generation": LAYOUT_UNKNOWN,
        "bucket": _CONTAINER_BUCKETS.get(container),
        "canonical_target": None,
        "expected_kind": None,
        "kind_mismatch": False,

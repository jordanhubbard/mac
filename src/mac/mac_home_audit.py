...
below are re-derived from ``docs/home-consolidation.md`` §1. Its one capability —
listing the gateway home and flagging entries outside that allow-list — is kept
alive by :func:`audit_gateway_home`, which reads the very same
:data:`CANONICAL_LAYOUT` gateway bucket instead of a second, drifting copy.
"""
...
#: Report schema for the single-home gateway audit (:func:`audit_gateway_home`).
GATEWAY_HOME_AUDIT_SCHEMA = "mac.gateway_home_audit.v1"
...
def _classify_children(
    ...
    *,
    ...
    """List one container directory and classify its children against ``allowed``.

    The single technique behind both depth-2 bucket auditing and the gateway-home
    audit: enumerate one level, and anything the allow-list does not name is
    drift. Nothing below is recursed into — an audit that walked a whole home
    would spend its time inside ``journal/`` and ``backups/``, whose contents are
    dated and legitimately unenumerable.
    """
    names, error = _list_dir(container)
    entries = [
        _make_entry(
            container / name,
            f"{rel_prefix}{name}",
            depth,
            CANONICAL if name in allowed else DRIFT,
            bucket,
            None,
        )
        for name in names
...
def audit_gateway_home(home: Optional[Path | str] = None) -> Dict[str, Any]:
    """Audit the gateway home against the canonical ``gateway`` allow-list.

    Wherever ``mac_paths.gateway_home()`` currently points — nested under the
    root already, or still pinned outside it on an un-migrated host — this lists
    it one level deep and classifies each entry as :data:`CANONICAL` or
    :data:`DRIFT`. It exists because the ``gateway/`` bucket of a pre-Phase-2
    root is empty (the gateway's own tree is elsewhere), so the allow-list would
    otherwise never be applied to the data it describes.

    Read-only and total, on the same terms as :func:`audit_mac_home`: a missing
    or unreadable home is a ``status``, never an exception.
    """
    spec = CANONICAL_LAYOUT["gateway"]
    home = Path(home) if home is not None else mac_paths.gateway_home()
    entries: List[Dict[str, Any]] = []
    error: Optional[str] = None
    home_exists = _exists(home)
    if not home_exists:
        status = STATUS_MISSING_ROOT
    elif not _is_dir(home):
        status = STATUS_NOT_A_DIRECTORY
    else:
        entries, error = _classify_children(
            home, spec.entries, rel_prefix="", depth=1, bucket=spec.name
        )
        status = STATUS_OK if error is None else STATUS_UNREADABLE_ROOT

    drift = [entry["relative_path"] for entry in entries if entry["classification"] == DRIFT]
    canonical = [entry["relative_path"] for entry in entries if entry["classification"] == CANONICAL]
    missing_expected = [name for name in spec.entries if not _exists(home / name)]
    return {
        "schema": GATEWAY_HOME_AUDIT_SCHEMA,
        "home_path": str(home),
        "audited_at": utcnow(),
        "home_exists": home_exists,
        "status": status,
        "status_detail": error,
        "expected": list(spec.entries),
        "entries": entries,
        "by_classification": {CANONICAL: canonical, DRIFT: drift},
        "missing_expected": missing_expected,
        "summary": {
            "entry_count": len(entries),

...
class StoreHelpersMixin:
    ...
    # Durable deploy-generation retirement facts. A worker's node-local
    # barrier file records a generation string; once the matching fleet
    # release epoch is aborted or committed the hub must be able to say
    # that generation is no longer live. Abort/commit behaviour is
    # unchanged; these helpers only record and look up.

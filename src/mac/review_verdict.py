...
#: The same summary rule, counting tests that RAN and failed.
_PYTEST_FAILED_SUMMARY_RE = re.compile(r"\b(\d+)\s+failed\b")
...
def _summary_count(output: str, pattern: "re.Pattern[str]") -> int:
    """Largest count ``pattern`` matches on a pytest summary rule line."""
    total = 0
...
def pytest_collection_error_count(output: str) -> int:
    ...
    for match in _PYTEST_INTERRUPTED_RE.finditer(text):
        total = max(total, int(match.group(1)))
    return max(total, _summary_count(text, _PYTEST_ERROR_SUMMARY_RE))


def pytest_failed_test_count(output: str) -> int:
    """How many tests RAN and failed, per the summary rule; 0 when unknown."""
    return _summary_count(output, _PYTEST_FAILED_SUMMARY_RE)
...
def classify_contract_run(
    ...
    # pytest's own summary outranks the signature scan.  The signatures are
    # substring matches over the WHOLE run, and a failing test prints its source
    # and locals -- so a red assertion inside a test that merely mentions
    # ModuleNotFoundError would otherwise be signed `infrastructure` and never
    # spend an attempt.  A summary saying "N failed" with zero collection errors
    # is pytest stating that the suite ran and disagreed; nothing incidental in
    # the log gets to overrule it.  (Same precedence the hub's
    # `_HUB_VERIFY_VERDICT_SIGNATURES` already applies to its own scan.)
    failed = pytest_failed_test_count(output or "")
    if failed > 0:
        return (
            HarnessOutcome.PASS,
            ReproducibilityOutcome.FAIL,
            "contract suite ran and reported %d failing test%s"

...
def detect_nvidia(
    ...
    detect_nvidia.last_physical = physical
    if not physical:
        return None
...
def _detect_hardware(
    ...
    detect_nvidia.last_physical = []
    physical_gpus: list[Dict[str, Any]] = []
    nvidia_probed = nvidia_query is not None or nvidia_list is not None
    gpu = None
    try:
        if nvidia_query is not None or nvidia_list is not None:
            query_text = _resolve_probe_text(
                nvidia_query, _nvidia_query_output
            )
            list_text = _resolve_probe_text(
                nvidia_list, lambda: _run(["nvidia-smi", "-L"])
            )
            ...
            gpu = detect_nvidia()
            physical_gpus = list(getattr(detect_nvidia, "last_physical", []) or [])
            nvidia_probed = bool(physical_gpus) or gpu is not None
    except Exception:  # noqa: BLE001 - detection must never break registration
        gpu = None
    if not gpu:
        try:

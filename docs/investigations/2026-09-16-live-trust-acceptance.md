...
The final qualified deployment transaction finalized, its hub epoch committed, and all three workers returned healthy with their rollout holds cleared. The original code canaries and worker-1 report ran after the earlier `3e498a6b` rollout; worker-3's repaired report ran after `03fcd3db`. Five ordinary canaries completed and were separately accepted: LRU behavior, trie behavior, token end offsets, worker-1's read-only report, and worker-3's read-only report. The last report passed 99 tests and mypy across 11 source files at canary revision `45f9ca7e6d45145bab881652f5571712291fd7cc`.
...
| Worker loss | One SIGKILL to worker-3's worker service at 13:03:39 UTC during the exact task sandbox's observed 45-second pause. | Accepted. Systemd restarted after five seconds; a new execution resumed the still-valid original lease and attempt. Full 99-test contract and report publication passed, with one current executor result and one publication. No manual restart, forced expiry or task reopen was required. Both task sandboxes were removed. |
...
|---|---|---|---|
| worker-3 LRU code | `task_883bd8474136bcf15b83fb258c00ad55` | 12.4 h | Yes |
| worker-2 trie code | `task_62896378399a0e2a8086962919e01907` | 12.52 h | Yes |
| worker-1 token-end code | `task_f569f8a6f6372704d432921ff98cc14f` | 12.66 h | Yes |
| worker-1 report | `task_00e09b814320a53699e442aa2debcbdd` | 12.79 h | Yes |
| worker-3 report | `task_e7953b07d4dbef7d229e2330fb3a4cd1` | 17.28 h | Yes |

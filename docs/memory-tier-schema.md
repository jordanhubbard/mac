...
```python
{
  ...
  "subject_type":    "task_summary" | "evidence_digest" | "conversation_thread" | "nap_summary",
...
The `summary` is the source text the embedding was computed from.
Storing it on the payload (not just on `memory_records` in SQLite) lets
the recall API render results without a SQLite round-trip per hit, at
the cost of duplicating a small string. Worth it.
...
| Daily, per agent | Nap consolidator (mem-08) summarizes the agent's last 24h of memory_records into one `nap_summary` row + one medium-tier vector |

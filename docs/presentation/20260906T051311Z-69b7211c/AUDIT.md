...
Audited at commit `69b7211c` on 2026-09-06T05:13:11Z. This audit supports
...
against the candidate tree. The release range is `v1.3.5..69b7211c` (9
...
| `docs/reference/cli.md`, `docs/reference/openapi.md` | not changed: no CLI flag, subcommand, or HTTP route was added, removed, or renamed in this range | generated references at `69b7211c` |
...
| All other current inventory rows | not changed by this range | source review against `v1.3.5..69b7211c` |
...
| Claim | Evidence |
|---|---|
| 9 commits since v1.3.5 | `git rev-list --count v1.3.5..69b7211c` |
| One local-only test failure (`tests/cli/test_task_table_column_widths.py::test_the_table_never_exceeds_the_terminal`), terminal-width dependent, does not reproduce in GitHub CI | local `make test` run at `69b7211c`; consistent with the same test's status noted during this session's fleet cutover work |
| `make lint`, `make docs-check` clean at `69b7211c` | local run at `69b7211c` (one formatting fix applied and committed pre-audit: `69b7211c` itself) |

#!/usr/bin/env python3
"""Build the MAC capabilities deck for commit e8040fec as a 16:9 .pptx.
...
def scale_slide() -> None:
    ...
    rows = [
        ("205,258", "lines of Python under src/", "212 modules in src/mac"),
        ("506", "test files under tests/", "plus fault-replay and contract suites"),
        ("414", "HTTP routes", "generated from the live OpenAPI schema"),
        ("125", "CLI verbs", "across project, task, agent and admin"),
        ("18", "book chapters", "every shell example executed by make docs-check"),
        ("31", "architecture decision records", "16 are Proposed, not shipped"),
...
def honesty_slide() -> None:
    ...
    items = [
        (
            "Sixteen ADRs are still Proposed, including 0017, 0018 and 0029.",
            "Token metering, the task graph, and the fleet-wide route ladder are designs with measured motivation — none of them has shipped. ADR 0016 is Accepted as a decision; the hub still drives RUNNING → NEEDS_REVIEW → REVIEWING.",
        ),
        (
...
census is this capture: blocked 378 and growing. The 165-of-355 dead-dependency finding is from

...
```bash
mac --db "$DOCS_DB" init
mac --db "$DOCS_DB" machine register bus-host --machine-id machine_bus
...
mac --db "$DOCS_DB" agentbus open agent_sender \
  --recipient-agent-id agent_receiver --topic tutorial \
  --stream-id stream_tutorial
mac --db "$DOCS_DB" agentbus append stream_tutorial agent_sender \
  --payload '{"message":"hello"}' --payload-encoding json --final
mac --db "$DOCS_DB" agentbus read stream_tutorial agent_receiver >/dev/null
mac --db "$DOCS_DB" agentbus close stream_tutorial agent_sender
mac integrations --help >/dev/null
```

...
| Rejection cause and `failure_class` | `reviews.status/reason` (`rejected`, `tests_failed`, `infrastructure`); review verdict manifest fields `verdict`, `review_axes`, `semantic_verdict`, `feedback`, `findings`; `task_history.detail.reason/review_verdict_axis/attempt_consumed/problems`; `tasks.metadata.review_attempt_accounting`; terminal `tasks.metadata.failure_class/salvage`. | `src/mac/review_verdict.py`; `docs/review-verdict-axes.md`; `src/mac/review_failure_classifier.py`; `src/mac/attempt_failure_classifier.py`; `src/mac/services.py` terminal attempt classification; `tests/test_review_verdict_three_axis.py`, `tests/test_review_failure_classifier.py`, `tests/test_attempt_failure_classifier.py`, `tests/test_review_protocol_rejection.py`. | Read the cause off `review_axes` — it is recorded by the producer, so nothing has to be reconstructed from prose. `classify_review_failure` still classifies review RETRACTIONS, which are not verdicts. For exhausted attempts, read the already-persisted `tasks.metadata.failure_class` and fall back to replaying `task_history` through `classify_attempt_failure` only for older rows. |

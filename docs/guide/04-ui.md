...
The Task view separates reported tests, operator acceptance, publication, and
recorded deployment evidence. Missing evidence stays unknown. It also shows
state-specific CLI commands to inspect, answer, stop, or diagnose the task,
plus the required authority. Run them using an authenticated profile for the
same hub. The hub rereads state and enforces permissions when the command runs;
a command displayed earlier may no longer be applicable.

This is an actionable handoff while the console remains read-only. See
[the trust workflow](06-trust-workflow.md) for acceptance and recovery.

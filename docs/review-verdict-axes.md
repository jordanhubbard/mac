...
`classify_contract_run` reads a run in a fixed order, and the order is the
safety property:

1. **Collection errors** → `infrastructure`. pytest counts them separately from
   failures precisely because they mean the tests never ran.
2. **A summary saying `N failed`** → `tests_failed`. pytest is stating that the
   suite ran and disagreed, and nothing incidental in the log overrules it.
   This outranks step 3 because the harness signatures are substring matches
   over the *whole* run and a failing test prints its source and locals —
   several tests in this repository contain the literal string
   `ModuleNotFoundError`, so without this precedence a real regression in one
   of them would be signed `infrastructure` and retry forever.
3. **A harness signature** (bootstrap failure, transport fault, unprovisionable
   database) → `infrastructure`.
4. **Anything else** → `tests_failed`, failing closed.

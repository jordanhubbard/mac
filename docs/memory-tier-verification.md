...
### The alerts are evaluated on a clock, not on request

Computing an alert is not monitoring. Before this change every caller of
`memory_health` was operator-driven — the CLI, the HTTP route, two facades —
so the alerts above would have been correct on 2026-07-26 and still nobody
would have seen them until someone typed the command 27 days later. That is
the same failure the stranding detector had (312 unresolved episodes, 30
hours without a run, because only `mac task throughput` opened one).

`ControlPlane.memory_health_tick` runs the snapshot from the hub's own clock,
inside the dispatcher tick, and emits:

| Event | When |
|---|---|
| `memory.alert` | one per **critical** alert, with the code, message and collection |
| `memory.health_tick_ran` | every run, healthy or not — a monitor that is silent when healthy cannot be told apart from one that is not running |
| `memory.health_tick_failed` | the probe raised; the tick continues regardless |

It rides the dispatcher tick rather than taking a timer of its own, for the
same reason promotion rides the nap: the job that died on 2026-07-25 was a
*separate* scheduled job that the Hermes → OpenClaw migration did not carry
over. It is self-throttled to `MAC_MEMORY_HEALTH_TICK_SECONDS` (default 900)
because each run costs a Qdrant round-trip and a bounded payload scan — far
below the 24h staleness threshold it watches, so the throttle can never be
the reason an alert is late. `MAC_MEMORY_INGESTION_MAX_AGE_HOURS` (default
24) sets that threshold for the scheduled path. A hub with no Qdrant
configured skips silently: no vector store means no ingestion to have
stopped.

...
manifest = {
    ...
    "deploy": {
        ...
        "worker_claim_only_canary_tasks": os.environ.get("WORKER_CLAIM_ONLY_CANARY_TASKS") or None,
...
def private_bytes(path: str) -> bytes:
    ...
    try:
        ...
        if len(raw) != before.st_size or (
            ...
            raise SystemExit("existing phase-2 rollback intent changed while reading")
        return raw
    finally:
        os.close(descriptor)
...
venv = artifacts.get("venv") if isinstance(artifacts, dict) else None
hermes = artifacts.get("hermes") if isinstance(artifacts, dict) else None
...
hermes_backup = hermes.get("backup") if isinstance(hermes, dict) else None
openclaw_backup = artifacts.get("openclaw_backup") if isinstance(artifacts, dict) else None
openclaw_existed = artifacts.get("openclaw_existed") if isinstance(artifacts, dict) else None
generation_re = re.compile(r"[A-Za-z0-9][A-Za-z0-9._:+-]{0,511}\Z")
revision_re = re.compile(r"[0-9a-f]{40}\Z")
...
PYTHONPATH="$SRC_DIR/src:${PYTHONPATH:-}" "$PY" -m mac.deploy_env write-mac-env \
  ...
  "$WORKER_REQUIRED_METADATA" "$WORKER_CLAIM_ONLY_CANARY_TASKS" \
...
EOF
  ...
  # Route to the configured gateway implementation. OpenClaw is the sole chat
  # gateway; ``none`` is a pure worker with no chat gateway at all.
  case "${MAC_CHAT_GATEWAY_IMPL:-openclaw}" in
    ...
    *)
      die "unsupported Linux gateway implementation: ${MAC_CHAT_GATEWAY_IMPL}" ;;
  esac
}
...
case "${MAC_WORKER_CLAIM_ONLY_CANARY_TASKS:-}" in
  1|true|TRUE|yes|YES|on|ON)
    common+=(--claim-only-canary-tasks)
    ;;
esac
...
install_supervisord_service() {
  ...
  openclaw|"")
    ...
    gateway_program="[program:$OPENCLAW_SUPERVISORD_PROG]
...
EOF
  ...
  # the gateway/agent start, mirroring the systemd and launchd paths. Without
  # this the supervisord path left the hub vault empty,
...
EOF
  ...
  none)
    ...
    ;;
  *) die "unsupported launchd gateway implementation: ${MAC_CHAT_GATEWAY_IMPL}" ;;
  esac
...
EOF
  ...
  mac_launchd_transaction_commit
  darwin_clear_auxiliary_restore
  log "stock OpenClaw verified as exclusive launchd gateway"
}

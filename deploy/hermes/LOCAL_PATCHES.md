...
| `fts5-orphan-schema-recovery.patch` | Makes `SessionDB._init_schema()` in `hermes_state.py` self-heal an orphaned FTS5 schema — a `messages_fts` / `messages_fts_trigram` virtual-table declaration whose backing shadow tables (`*_data`/`*_idx`/`*_docsize`/`*_config`) were lost or corrupted. Previously the startup probe only handled `no such table: <vtable>` (fresh create); an orphan surfaced as `vtable constructor failed` / `malformed database schema` and aborted startup on an otherwise-usable DB. The fix classifies orphan/corrupt signatures, drops the orphaned vtable + triggers + stray shadow tables (bumping the schema cookie so the live connection reparses), recreates the index from the canonical DDL, and backfills from `messages`. Touches only `hermes_state.py`; applies independently of the other patches. |

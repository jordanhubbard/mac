...
def test_resolve_binary_and_git_exclude_fallbacks(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(audit.subprocess, 'run', lambda *args, **kwargs: (_ for _ in ()).throw(OSError('git missing')))
    assert audit._git_exclude_path(repo) == repo / '.git' / 'info' / 'exclude'
    assert audit._git_exclude_path(tmp_path / 'plain') is None


def test_ensure_exclude_handles_absent_and_existing_content(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_truncate_and_command_error_records(tmp_path: Path) -> None:
    ...
    timed = audit._run_codegraph_command(['/bin/codegraph', 'sync'], cwd=tmp_path, timeout=1, runner=timeout)
    ...
    failed = audit._run_codegraph_command(['/bin/codegraph', 'sync'], cwd=tmp_path, timeout=1, runner=missing)
...
def test_run_audit_unavailable_and_exclude_warning(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(audit, 'ensure_codegraph_git_exclude', lambda _repo: (_ for _ in ()).throw(OSError('read-only')))
    calls: list[str] = []
...
def test_pass_check_and_manifest_problem_variants() -> None:
    ...
    check = audit.codegraph_audit_check({'status': 'fail', 'reason': 'bad', 'commands': [{'argv': ['codegraph', 'sync']}, 'ignore'], 'relevant_files': ['src/a.py']})

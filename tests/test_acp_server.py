...
def test_prompt_turn_thought_tool_update_and_permission_error() -> None:
    ...
    peer.pending = SimpleNamespace(result=lambda _timeout: (_ for _ in ()).throw(RemoteError(JSONRPCError(code=-1, message='unsupported'))))
    result = turn.request_permission({'toolCallId': 'call'}, [PermissionOption('allow', 'Allow', 'allow_once')])
...
def test_backend_normalization_echo_and_session_load_paths() -> None:
    ...
    acp = server.ACPAgentServer(peer, lambda _turn: StopReason.END_TURN, agent_capabilities=AgentCapabilities(load_session=True))
...
def test_run_turn_remote_and_internal_errors() -> None:
    ...
    acp._active_turns = 1
    acp._backend = SimpleNamespace(run_prompt=lambda _turn: (_ for _ in ()).throw(RemoteError(JSONRPCError(code=-1, message='remote'))))
    ...
    session.current_turn = turn
    acp._active_turns = 1
    acp._backend = SimpleNamespace(run_prompt=lambda _turn: (_ for _ in ()).throw(RuntimeError('boom')))

...
# ---------------------------------------------------------------------------
# Renaming the outcome must not silently un-teach the fleet
# ---------------------------------------------------------------------------


def test_a_red_suite_is_still_learned_from_under_its_new_outcome_name():
    """Splitting the verdict renamed the lesson; the reader must follow.

    A red contract suite used to be filed as ``review_rejected``, which the
    dream pipeline recognises as a pitfall.  It now arrives as
    ``review_tests_failed``: an outcome the pipeline does not recognise is
    dropped entirely, so the split would have quietly stopped the fleet
    learning from failing suites -- the opposite of the intent.
    """
    from mac.dreaming import pipeline

    assert "review_tests_failed" in pipeline._FAILURE_OUTCOMES
    # Never emitted as a lesson at all, and must not be classified as one if a
    # stale record ever carries it: a sandbox fault is not a fact about work.
    assert "review_infrastructure" not in pipeline._FAILURE_OUTCOMES
    assert "review_infrastructure" not in pipeline._SUCCESS_OUTCOMES


def test_a_harness_failure_writes_no_lesson_and_no_project_failure_record(cp):
    """The lesson writers are gated on attempt consumption, not on 'not approved'."""
    from mac.review_verdict import verdict_consumes_attempt

    assert verdict_consumes_attempt(ReviewVerdict.TESTS_FAILED.value)
    assert verdict_consumes_attempt(ReviewVerdict.REJECTED.value)
    assert not verdict_consumes_attempt(ReviewVerdict.INFRASTRUCTURE.value)
    assert not verdict_consumes_attempt(ReviewVerdict.APPROVED.value)

    _, reviewer, task, review, ev = _drive_to_review(cp, "infra-teaches-nothing")
    _submit(
        cp,
        review,
        reviewer,
    ...
    assert not cp.search_memory(
        record_type_prefix="deployment_learning"
    ), "a sandbox fault must not be distilled into a lesson"

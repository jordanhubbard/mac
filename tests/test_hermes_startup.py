...
def test_file_url_command_and_topology_helpers_cover_error_paths(tmp_path, monkeypatch):
    ...
    topology.write_text(json.dumps({'schema': 'topology', 'agent': 'agent', 'hub': {'agent': 'hub', 'url': 'http://user:pass@hub:8789?q=secret'}, 'shared_services': {'qdrant': {'url': 'http://qdrant:6333'}}}), encoding='utf-8')
...
@pytest.mark.parametrize('payload,expected', [('not-json', 'invalid_context'), ('[]', 'invalid_context')])
...
def test_runtime_context_contract_failure_matrix(tmp_path, monkeypatch):
    ...
    workspace.mkdir()
    context = build_runtime_context(agent_name='agent', fleet_name='fleet', mac_url='http://hub:8789', hermes_home=home, mac_home=tmp_path / 'mac', hermes_instance_id='hermes_agent', agent_id='agent_agent', workspace_path=workspace)
    ...
    missing_capability['session_capabilities']['capabilities'] = [item for item in missing_capability['session_capabilities']['capabilities'] if item.get('name') != 'web_search']
...
def test_session_capability_probes_record_failures(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(startup.subprocess, 'run', lambda *_args, **_kwargs: (_ for _ in ()).throw(OSError('no shell')))
    monkeypatch.setattr(startup.os, 'access', lambda *_args: False)
    capabilities = [{'name': 'shell_execution', 'required': True, 'command': 'sh'}, {'name': 'workspace_file_access', 'required': True, 'cwd': str(workspace)}, {'name': 'mac_api', 'required': True, 'endpoint': '<invalid-url>'}, {'name': 'web_search', 'required': True, 'environment': ['MISSING_SEARCH']}]
    report = startup._session_capability_availability(capabilities, workspace={'path': str(workspace), 'project_contract': {'path': str(contract), 'schema': 'wrong', 'required_commands': ['missing-command']}})
    assert report['ready'] is False
    assert 'project_contract' in report['missing']
    assert 'project_toolchain:missing-command' in report['missing']
    assert {'shell_execution', 'workspace_file_access', 'mac_api', 'web_search'} <= set(report['missing'])
...
def test_qdrant_and_firecrawl_report_failure_matrix(tmp_path, monkeypatch):
    ...
    topology.write_text('{}', encoding='utf-8')
    monkeypatch.setattr(startup, '_fetch_qdrant_collections', lambda *_args: (_ for _ in ()).throw(OSError('offline')))
...
def test_tokenhub_report_required_degraded_and_transport_matrix(monkeypatch):
    for name in ('TOKENHUB_URL', 'MAC_TOKENHUB_URL', 'MAC_REQUIRE_TOKENHUB', 'MAC_TOKENHUB_ALLOW_DEGRADED', 'TOKENHUB_API_KEY', 'TOKENHUB_AGENT_KEY', 'OPENAI_API_KEY', 'MAC_ROUTER_BACKEND'):
        monkeypatch.delenv(name, raising=False)
    ...
    monkeypatch.setattr(startup, '_fetch_tokenhub_json', lambda *_args: (_ for _ in ()).throw(OSError('offline')))

...
def test_memory_search_returns_all_records_when_no_filters(tmp_path):
    ...
    rc, records = _run(tmp_path, "memory", "search")
    assert rc == 0
...
def test_memory_search_filters_by_subject_type(tmp_path):
    ...
    rc, records = _run(tmp_path, "memory", "search", "--subject-type", "project")
    assert rc == 0
...
def test_memory_search_filters_by_subject_id(tmp_path):
    ...
    rc, records = _run(tmp_path, "memory", "search", "--subject-id", "worker-1")
    assert rc == 0
...
def test_memory_search_filters_by_record_type(tmp_path):
    ...
    rc, records = _run(tmp_path, "memory", "search", "--record-type", "note")
    assert rc == 0
...
def test_memory_search_filters_by_record_type_prefix(tmp_path):
    ...
    rc, records = _run(tmp_path, "memory", "search", "--record-type-prefix", "dream:")
    assert rc == 0
    assert len(records) == 2
...
def test_memory_search_filters_by_created_by(tmp_path):
    ...
    rc, records = _run(tmp_path, "memory", "search", "--created-by", "hub")
    assert rc == 0
...
def test_memory_search_limit_flag(tmp_path):
    ...
    rc, records = _run(tmp_path, "memory", "search", "--limit", "2")
    assert rc == 0
    assert len(records) == 2
...
def test_memory_search_order_desc(tmp_path):
    ...
    rc, records = _run(tmp_path, "memory", "search", "--order", "desc")
    assert rc == 0
    assert len(records) == 2
...
def test_memory_decay_returns_expected_schema_fields(tmp_path):
    ...
    rc, result = _run(tmp_path, "memory", "decay", "--ttl-days", "30")
    assert rc == 0
...
def test_memory_decay_apply_flag_sets_dry_run_false(tmp_path):
    ...
    rc, result = _run(tmp_path, "memory", "decay", "--apply")
    assert rc == 0
...
def test_memory_decay_respects_ttl_days_flag(tmp_path):
    ...
    rc, result = _run(tmp_path, "memory", "decay", "--ttl-days", "7")
    assert rc == 0
...
def test_memory_decay_limit_flag(tmp_path):
    ...
    rc, result = _run(tmp_path, "memory", "decay", "--limit", "100")
    assert rc == 0
    assert result["schema"] == "mac.memory_decay.v1"
...
def test_memory_health_nap_interval_flag_accepted(tmp_path):
    ...
    rc, result = _run(tmp_path, "memory", "health", "--nap-interval-hours", "12")
    assert rc == 0
    assert result["schema"] == "mac.memory_health.v1"
...
def test_memory_recall_missing_qdrant_raises_error(tmp_path, monkeypatch):
    ...
    try:
        rc, result = _run(tmp_path, "memory", "recall", "what did worker-1 learn about the mesh?")
...
def test_memory_recall_query_positional_arg_parsed(tmp_path, monkeypatch):
    ...
    try:
        rc, result = _run(tmp_path, "memory", "recall", "hub deployment status")
...
def test_memory_summarize_actions_dry_run_returns_schema(tmp_path):
    ...
    rc, result = _run(tmp_path, "memory", "summarize-actions", "--dry-run")
    assert rc == 0

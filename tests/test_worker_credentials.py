...
def test_fleet_deploy_completes_bound_vm_credential_rollout() -> None:
    ...
    assert '"principal_mode":credential_mode' in hub_open
    assert "MAC_DEPLOY_WORKER_CREDENTIAL_MODE:-current" in script
    assert 'issue_pending_worker_credential "$agent" "$hub_agent"' in hub_open
    assert 'install_pending_worker_credential "$agent" "$supervisor"' in apply_worker
    assert 'open --epoch "$COHORT_EPOCH_ID" || return 1' in hub_open

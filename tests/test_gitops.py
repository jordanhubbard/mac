...
def _target(tmp_path: Path, **extra):
    values = {'worktree': tmp_path, 'canonical_remote_url': 'https://github.com/org/repo.git', 'remote': 'https://github.com/org/repo.git', 'remote_display': 'https://github.com/org/repo.git', 'canonical_branch': 'main', 'destination_branch': 'task/change', 'prepared_base_sha': 'a' * 40, 'task_head_sha': 'b' * 40, 'isolated_ref': 'refs/mac/publication/test', 'git_common_dir': tmp_path, 'lock_path': tmp_path / 'lock'}
    values.update(extra)
...
def test_ref_host_token_auth_and_redaction_edges(monkeypatch) -> None:
    ...
    assert gitops.inject_git_remote_auth('ssh://host/repo') == 'https://x-access-token:fallback@host/repo'
...
def test_resolve_publication_target_validation_sequence(monkeypatch, tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match='worktree is missing'):
        gitops.resolve_canonical_publication_target(worktree=missing, canonical_remote='/remote', canonical_branch='main', destination_branch='task', prepared_base_sha='a' * 40, isolation_key='key')
    monkeypatch.setattr(gitops, '_run_git', lambda *_a: _cp(1))
    with pytest.raises(ValueError, match='not a git worktree'):
        gitops.resolve_canonical_publication_target(worktree=tmp_path, canonical_remote='/remote', canonical_branch='main', destination_branch='task', prepared_base_sha='a' * 40, isolation_key='key')
...
@pytest.mark.parametrize(('results', 'message'), [([_cp(stdout='true'), _cp(), _cp(), _cp(1)], 'prepared repository base is not a commit'), ([_cp(stdout='true'), _cp(), _cp(), _cp(stdout='a' * 40), _cp(1, stderr='head')], 'could not resolve task HEAD'), ([_cp(stdout='true'), _cp(), _cp(), _cp(stdout='a' * 40), _cp(stdout='b' * 40), _cp(1)], 'not an ancestor'), ([_cp(stdout='true'), _cp(), _cp(), _cp(stdout='a' * 40), _cp(stdout='b' * 40), _cp(), _cp(1)], 'isolated publication ref')])
...
@pytest.mark.parametrize(('results', 'push', 'message'), [([_cp(1, stderr='head')], False, 'resolve task HEAD'), ([_cp(stdout='c' * 40)], False, 'HEAD changed'), ([_cp(stdout='b' * 40), _cp(1)], False, 'prepared repository base'), ([_cp(stdout='b' * 40), _cp(), _cp(1, stderr='fetch'), _cp()], False, 'fetch of canonical'), ([_cp(stdout='b' * 40), _cp(), _cp(), _cp(1, stderr='ref'), _cp()], False, 'did not resolve'), ([_cp(stdout='b' * 40), _cp(), _cp(), _cp(stdout='c' * 40), _cp(), _cp(1, stderr='diff'), _cp()], False, 'compute canonical diff'), ([_cp(stdout='b' * 40), _cp(), _cp(), _cp(stdout='c' * 40), _cp(), _cp(stdout='file\n'), _cp(), _cp(1, stderr='push')], True, 'git push'), ([_cp(stdout='b' * 40), _cp(), _cp(), _cp(stdout='c' * 40), _cp(), _cp(stdout='file\n'), _cp(), _cp(stdout='ok'), _cp(stdout='wrong refs/heads/task')], True, 'remote branch verification')])
...
def test_http_post_error_and_pull_request_existing_paths(monkeypatch) -> None:
    ...
    assert gitops.open_pull_request('https://github.com/org/repo', 'task', base='main', github_token='token') is existing
    monkeypatch.setattr(gitops, '_find_existing_pr', lambda *_a: None)
    with pytest.raises(RuntimeError, match='already exists'):
        gitops.open_pull_request('https://github.com/org/repo', 'task', base='main', github_token='token')

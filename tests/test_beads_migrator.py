...
def test_migrate_records_lookup_create_and_memory_failures(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr(
        beads,
        "_create_task_from_bead",
        lambda *_a, **_k: (_ for _ in ()).throw(RuntimeError("create failed")),
    )
    monkeypatch.setattr(
        beads,
        "_import_memory",
        lambda *_a, **_k: (_ for _ in ()).throw(RuntimeError("memory failed")),
    )
    report = beads.migrate(
        ...
        project="p",
        emit_tickets=False,
        memories={"schema_version": "1", "rule": "value"},
    )
...
def test_read_beads_memories_cli_failure_and_success(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr(
        beads.subprocess, "run", lambda *_a, **_k: (_ for _ in ()).throw(OSError("missing"))
    )
    assert beads.read_beads_memories_via_cli(tmp_path) == {}
    monkeypatch.setattr(
        beads.subprocess, "run", lambda *_a, **_k: subprocess.CompletedProcess([], 2, "", "bad")
    )
    assert beads.read_beads_memories_via_cli(tmp_path) == {}
    monkeypatch.setattr(
        beads.subprocess, "run", lambda *_a, **_k: subprocess.CompletedProcess([], 0, "bad", "")
    )
    assert beads.read_beads_memories_via_cli(tmp_path) == {}
    monkeypatch.setattr(
        beads.subprocess, "run", lambda *_a, **_k: subprocess.CompletedProcess([], 0, "[]", "")
    )
    assert beads.read_beads_memories_via_cli(tmp_path) == {}
    monkeypatch.setattr(
        beads.subprocess,
        ...
        lambda *_a, **_k: subprocess.CompletedProcess(
            [], 0, '{"schema_version":1,"rule":2,"text":"ok"}', ""
        ),
    )

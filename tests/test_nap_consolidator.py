...
def test_nap_cycle_skips_busy_agent_without_raising(cp):
    ...
    with patch.object(cp, "begin_nap", side_effect=ValidationError("agent holds an active lease")):
        out = cp.run_nap_cycle(agent.id)
...
def test_consolidate_validation_blank_summary_and_disabled_dreams() -> None:
    ...
    service = nap.NapConsolidatorService(
        store=cp.store, memory=cp.memory, summarizer_fn=lambda _records, _context: "   "
    )
...
def test_consolidate_records_summarizer_and_dreamer_errors() -> None:
    ...
    result = nap.NapConsolidatorService(
        store=cp.store, memory=cp.memory, summarizer_fn=fail, dreamer_fn=fail

"""Worker attestation-key drift diagnostics.
...
The worker may verify its installed key but key replacement belongs to the
...
def test_heal_reports_controller_recovery_without_rotating(tmp_path, monkeypatch):
    ...
    assert "K1-stale" in env_file.read_text(encoding="utf-8")
...
def test_heal_is_rate_limited(tmp_path, monkeypatch):
    ...
    # a second rejection within the window must not even repeat the probe
    assert w._heal_attestation_key() is False

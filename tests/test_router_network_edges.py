...
def _http_error(body=b"", *, code=400, reason="bad"):
    return urllib.error.HTTPError(
        "https://provider.test", code, reason, {}, io.BytesIO(body)
    )
...
def test_payload_token_floor_and_context_edges(monkeypatch) -> None:
    ...
    assert router_app._ensure_max_tokens_floor({"max_tokens": "bad"}, "/chat/completions")["max_tokens"] == 32000
    ...
    assert router_app._body_route_context(body) == {
        "agent_id": "first", "task_id": "task", "fleet": "fleet"
    }
...
def test_multipart_encoder_accepts_fields_and_file() -> None:
    content_type, body = router_app._encode_multipart({
        ...
        "file": {
            "name": "audio", "filename": "sample.wav",
            "content_type": "audio/wav", "b64": "aGVsbG8=",
        },
    })
...
def test_urllib_forwarder_json_binary_and_multipart(monkeypatch) -> None:
    ...
    monkeypatch.setattr(router_app.urllib.request, "urlopen", lambda req, **_k: seen.append(req) or next(responses))
    ...
    assert router_app.urllib_forwarder(_provider(), "/json", {}, binary_ok=True)[1] == {"job": "done"}
...
def test_urllib_forwarder_error_shapes(monkeypatch) -> None:
    ...
    monkeypatch.setattr(router_app.urllib.request, "urlopen", lambda *_a, **_k: (_ for _ in ()).throw(next(errors)))
    assert router_app.urllib_forwarder(_provider(), "/x", {})[0] == 401
    assert router_app.urllib_forwarder(_provider(), "/x", {})[1]["error"]["message"] == "plain failure"
    assert router_app.urllib_forwarder(_provider(), "/x", {})[1]["error"]["message"] == "unavailable"
...
def test_stream_forwarder_success_http_errors_and_transport(monkeypatch) -> None:
    ...
    assert router_app.urllib_stream_forwarder(_provider(), "/stream", {})[1]["error"]["message"] == "plain"

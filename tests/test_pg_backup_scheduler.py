...
def test_default_on_only_for_postgres_hub():
    ...
    # explicit off
    assert (
        PgBackupConfig.from_env({"MAC_DATABASE_URL": PG, "MAC_PG_BACKUP_ENABLED": "0"}).enabled
        is False
    )
...
def test_verify_every_controls_restore_drill_cadence(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(pg_backup, "dump", fake_dump)
    cfg = PgBackupConfig(
        enabled=True, dsn=PG, out_dir=str(tmp_path / "b"), keep_last=5, verify_every=3
    )

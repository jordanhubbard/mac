...
def test_snapshot_captures_state_and_manifest(tmp_path):
    ...
    m = journal.snapshot(home=home, root=root, date="2026-06-04", agent_id="hosta", run_hook=False)
    ...
    assert m["agent_id"] == "hosta" and m["version"] == "mac.agent_journal.v1"
...
def test_backup_hook_runs_with_env(tmp_path, monkeypatch):
    ...
    m = journal.snapshot(home=home, root=root, date="2026-06-04", agent_id="hostc")

    assert m["hook"]["ran"] is True and m["hook"]["exit_code"] == 0
    assert marker.read_text().strip() == "hostc 2026-06-04"
...
def test_hook_failure_does_not_break_snapshot(tmp_path, monkeypatch):
    ...
    m = journal.snapshot(home=home, root=root, date="2026-06-04", agent_id="hosta")
...
def test_list_journals(tmp_path):
    ...
    root = tmp_path / ".mac" / "journal"
    _make_agent_home(home)
    journal.snapshot(home=home, root=root, date="2026-06-03", agent_id="hosta", run_hook=False)
    journal.snapshot(home=home, root=root, date="2026-06-04", agent_id="hosta", run_hook=False)
...
def test_restore_reverts_state_and_keeps_safety_backup(tmp_path):
    ...
    assert "I'm Hosta." in (home / "SOUL.md").read_text()      # restored

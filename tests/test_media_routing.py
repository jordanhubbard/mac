...
def test_nvidia_genai_request_flux_schnell_dialect():
    path, body = nvidia_genai_request(
        "image.generate", {"prompt": "a duck"}, "black-forest-labs/flux.1-schnell"
    )
...
def test_nvidia_genai_request_stability_dialect():
    _, body = nvidia_genai_request(
        "image.generate", {"prompt": "x"}, "stabilityai/stable-diffusion-xl"
    )
...
def test_nvidia_genai_response_normalizes_to_artifacts():
    ...
    assert nvidia_genai_response(200, {"artifacts": [{"base64": "Z"}]}) == {
        "artifacts": [{"base64": "Z"}]
    }
...
def test_media_endpoint_generates_and_normalizes(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        _fake_urlopen_factory(
            captured, {"flux.1-schnell": (200, b'{"artifacts":[{"base64":"DUCKB64"}]}')}
        ),
    )
    app = FastAPI()
    env = {
...
def test_media_endpoint_rejects_path_traversal_model(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "urlopen",
        _fake_urlopen_factory(
            captured, {"flux.1-schnell": (200, b'{"artifacts":[{"base64":"X"}]}')}
        ),
    )
    app = FastAPI()
    env = {
    ...
    assert router_app.mount_router(app, env=env, secret_resolver={"nvidia-image": "K"}.get) is True
    r = TestClient(app).post(
        "/v1/media/image.generate", json={"prompt": "x", "model": "../../etc/passwd"}
    )
...
def test_openai_images_adapter_request_and_response():
    path, body = openai_images_request(
        "image.generate", {"prompt": "x", "width": 1024, "height": 1024}, "dall-e-3"
    )
    ...
    assert openai_images_response(200, {"data": [{"b64_json": "AB"}]}) == {
        "artifacts": [{"base64": "AB"}]
    }
...
def test_fal_adapter_request_and_url_response():
    path, body = fal_request(
        "image.generate", {"prompt": "x", "seed": 7, "steps": 4, "n": 2}, "fal-ai/flux/schnell"
    )
    assert path == "/fal-ai/flux/schnell"
    assert (
        body["prompt"] == "x"
        and body["seed"] == 7
        and body["num_inference_steps"] == 4
        and body["num_images"] == 2
    )
    # FAL returns image URLs -> canonical `url` artifacts (the agent downloads them)
    assert fal_response(200, {"images": [{"url": "https://cdn/x.png"}]}) == {
        "artifacts": [{"url": "https://cdn/x.png"}]
    }
...
def test_mount_forwards_with_binding_auth_scheme(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "urlopen",
        _fake_urlopen_factory(
            captured, {"flux/schnell": (200, b'{"images":[{"url":"https://cdn/x.png"}]}')}
        ),
    )
    app = FastAPI()
    env = {
...
def test_media_bindings_from_agents_priority_and_offline_skip():
    agents = [
        {
            ...
            "resources": {
                "media_routes": [
                    {
                        "op": "image.generate",
                        "base_url": "http://bw:8189",
                        "adapter": "openai_images",
                        "priority": 1,
                    },
                    {
                        "op": "image.generate",
                        "base_url": "http://bw:8190",
                        "adapter": "passthrough",
                        "priority": 0,
                    },
                ]
    ...
    assert [b.base_url for b in table["image.generate"]] == [
        "http://bw:8190",
        "http://bw:8189",
    ]  # priority asc; offline dropped
...
def test_compose_prefers_live_agent_then_static():
    static = {
        "image.generate": [
            MediaBinding("nvidia", "https://nv", "secret:k", "m", "nvidia_genai", 180.0)
        ]
    }
    agent = {
        "image.generate": [MediaBinding("bw", "http://bw:8189", "", "sdxl", "openai_images", 180.0)]
    }
    assert [b.provider for b in compose_media_table(static, agent, "image.generate")] == [
        "bw",
        "nvidia",
    ]
...
def test_media_endpoint_prefers_live_agent_over_static(monkeypatch):
    ...
    assert (
        router_app.mount_router(
            app, env=env, secret_resolver={}.get, media_agent_table_provider=agent_table
        )
        is True
    )
    r = TestClient(app).post("/v1/media/image.generate", json={"prompt": "x"})
...
def _gpu_agent(name, accel, gpu_name, op_url="http://h:8189"):
    return {
        ...
        "health_status": "healthy",
        "resources": {
            "hardware": {"accelerator": accel, "gpu": {"name": gpu_name}},
            "media_routes": [
                {"op": "image.generate", "base_url": op_url, "adapter": "openai_images"}
            ],
        },
    }
...
def _gpu_agent_vram(name, accel, vram_mb, url):
    return {
        ...
        "health_status": "healthy",
        "resources": {
            "hardware": {"accelerator": accel, "gpu": {"name": name, "vram_mb": vram_mb}},
...
def test_same_tier_ordered_by_vram_descending():
    ...
    assert urls == [
        "http://big:8189",
        "http://small:8189",
    ]  # bigger VRAM first within the cuda tier


def test_binding_carries_accelerator_rank():
    agents = [
        _gpu_agent_vram("natasha", "cuda", 0, "http://natasha:8189"),
        {
            "name": "mac",
            "status": "idle",
            "health_status": "healthy",
            "resources": {
                "hardware": {"accelerator": "metal", "memory_mb": 64000},
                "media_routes": [
                    {
                        "op": "image.generate",
                        "base_url": "http://mac:8189",
                        "adapter": "openai_images",
                    }
                ],
            },
    ...
    assert (
        ranks["http://natasha:8189"] == 0 and ranks["http://mac:8189"] == 1
    )  # cuda tier 0, metal tier 1
...
def test_dispatch_order_least_loaded_within_top_tier():
    ...
    assert [b.base_url for b in dispatch_order(bindings, {})][0] in (
        "http://a:8189",
        "http://b:8189",
    )
...
def test_mount_balances_across_two_gpu_agents(monkeypatch):
    ...
    assert (
        router_app.mount_router(
            app,
            env={"MAC_ROUTER_BACKEND": "inproc"},
            secret_resolver={}.get,
            media_agent_table_provider=agent_table,
        )
        is True
    )
    ...
    providers = {
        client.post("/v1/media/image.generate", json={"prompt": "x"}).json()["provider"]
        for _ in range(2)
    }
...
def test_op_is_binary():
    assert (
        op_is_binary("audio.tts") and op_is_binary("audio.music") and op_is_binary("video.generate")
    )
...
def test_audio_music_request_shaping():
    path, body = audio_music_request(
        "audio.music", {"prompt": "lofi", "duration": 5}, "musicgen-small"
    )
    assert path == "/audio/music"
    assert (
        body["prompt"] == "lofi" and body["duration"] == 5.0 and body["model"] == "musicgen-small"
    )
...
def test_audio_transcription_request_is_multipart():
    path, body = openai_audio_transcription_request(
        "audio.asr", {"audio": "QUJD"}, "whisper-large-v3"
    )
    ...
    ctype, body = _encode_multipart(
        {
            ...
            "file": {
                ...
                "b64": base64.b64encode(b"PCMDATA").decode(),
            },
        }
    )
...
def test_media_endpoint_audio_asr_multipart(monkeypatch):
    ...
    assert (
        router_app.mount_router(
            app, env={"MAC_ROUTER_BACKEND": "inproc", "MAC_ROUTER_MEDIA_JSON": media_json}
        )
        is True
    )
...
def test_video_generate_request_shaping():
    path, body = video_generate_request(
        "video.generate", {"prompt": "a cat", "duration": 4}, "animatediff"
    )
...
def test_video_generate_response_async_and_sync():
    assert video_generate_response(200, {"job_id": "vjob_1", "status": "running"}) == {
        "job_id": "vjob_1",
        "status": "running",
    }
    # synchronous fallback (a server that returns an artifact directly)
    assert video_generate_response(200, {"data": [{"b64_json": "B"}]}) == {
        "artifacts": [{"base64": "B"}]
    }
    ...
    def fake_urlopen(req, timeout=60.0):
        ...
        if "/video/jobs/vjob_42" in url:
            return _Resp(
                {
                    "status": "completed",
                    "artifacts": [{"base64": "GIFDATA", "content_type": "image/gif"}],
                }
            )

...
def test_gateway_children_are_attributed_to_the_gateway_bucket_at_either_position(tmp_path):
    """A child is filed under the bucket that owns its container, not its parent's name."""
    legacy_root = _legacy_tree(tmp_path / "legacy")
    _touch(legacy_root / "openclaw" / "SOUL.md")
    legacy = audit_mac_home(legacy_root)

    canonical_root = _canonical_tree(tmp_path / "canonical")
    _touch(canonical_root / "gateway" / "openclaw" / "SOUL.md")
    canonical = audit_mac_home(canonical_root)

    assert _entry_by_path(legacy, "openclaw/SOUL.md")["bucket"] == "gateway"
    assert _entry_by_path(canonical, "gateway/openclaw/SOUL.md")["bucket"] == "gateway"
    # Bucket members keep their own bucket.
    assert _entry_by_path(canonical, "ledger/mac.db")["bucket"] == "ledger"

...
def test_file_named_like_bucket_is_not_descended(tmp_path):
    root = tmp_path / "file-bucket"
    root.mkdir()
    _touch(root / "ledger")
    report = audit_mac_home(root)
    by_path = _by_path(report)
    assert by_path["ledger"]["classification"] == CLASS_CANONICAL
    assert by_path["ledger"]["kind"] == "file"
    assert all(not path.startswith("ledger/") for path in by_path)


def test_gateway_allow_list_reuses_hermes_known_names():
    from mac.hermes_home_audit import HERMES_KNOWN_TOP_LEVEL

    assert HERMES_KNOWN_TOP_LEVEL <= mha.GATEWAY_CHILD_KNOWN
    assert "openclaw" in mha.GATEWAY_CHILD_KNOWN


def test_nested_file_and_directory_kinds(tmp_path):
    root = tmp_path / "kinds"
    root.mkdir()
    _touch(root / "secrets", directory=True)
    _touch(root / "secrets" / "mac.env")
    _touch(root / "secrets" / "ssh", directory=True)
    by_path = _by_path(audit_mac_home(root))
    assert by_path["secrets/mac.env"]["kind"] == "file"
    assert by_path["secrets/ssh"]["kind"] == "directory"
    assert by_path["secrets/ssh"]["classification"] == CLASS_CANONICAL

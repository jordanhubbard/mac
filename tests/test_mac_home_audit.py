...
def test_the_module_never_imports_the_vendored_hermes_runtime():
    # Checked against the parsed import graph, not a substring sweep: prose about
    # the ancestor auditor must not be able to fail an import assertion.
    ...
    assert not [name for name in imported if "_hermes" in name.split(".")], sorted(imported)


def test_the_module_contains_no_write_verbs():
    source = Path(mac_home_audit.__file__).read_text(encoding="utf-8")
...
# --- the gateway home, audited where it actually lives ---------------------


def test_the_gateway_home_audit_uses_the_canonical_gateway_allow_list(tmp_path):
    # The point of auditing the gateway home separately: on a pre-Phase-2 root
    # the `gateway/` bucket does not exist, so this allow-list would otherwise
    # never meet the tree it describes.
    home = _build(tmp_path / "gateway", "SOUL.md", "config.yaml", "abandoned.log", dirs=("memories",))

    report = audit_gateway_home(home)

    assert report["status"] == "ok"
    assert sorted(report["by_classification"][CANONICAL]) == ["SOUL.md", "config.yaml", "memories"]
    assert report["by_classification"][DRIFT] == ["abandoned.log"]
    assert _entry(report, "abandoned.log")["bucket"] == "gateway"
    assert report["expected"] == list(CANONICAL_LAYOUT["gateway"].entries)


def test_the_gateway_home_audit_reports_missing_expected_entries(tmp_path):
    home = _build(tmp_path / "gateway", "SOUL.md")

    report = audit_gateway_home(home)

    assert "SOUL.md" not in report["missing_expected"]
    assert "state.db" in report["missing_expected"]
    assert report["summary"] == {
        "entry_count": 1,
        "canonical_count": 1,
        "drift_count": 0,
...
def test_the_gateway_home_audit_has_its_own_schema_and_is_json_serialisable(tmp_path):
    import json

    report = audit_gateway_home(_build(tmp_path / "gateway", "SOUL.md"))

    assert report["schema"] == GATEWAY_HOME_AUDIT_SCHEMA == "mac.gateway_home_audit.v1"
    assert report["home_path"] == str(tmp_path / "gateway")
    assert report["home_exists"] is True
    assert report["audited_at"].endswith("+00:00")
    assert json.loads(json.dumps(report)) == report


def test_a_missing_gateway_home_is_reported_and_nothing_is_created(tmp_path):
    report = audit_gateway_home(tmp_path / "never-installed")

    assert report["status"] == "missing_root"
    assert report["home_exists"] is False
    assert report["entries"] == []
    assert report["missing_expected"] == list(CANONICAL_LAYOUT["gateway"].entries)
    assert list(tmp_path.iterdir()) == []


def test_a_gateway_home_that_is_a_file_is_reported_not_raised(tmp_path):
...
@pytest.mark.skipif(os.geteuid() == 0, reason="root bypasses directory permissions")
def test_an_unreadable_gateway_home_is_reported_with_detail(tmp_path):
    home = _build(tmp_path / "gateway", "SOUL.md")
    os.chmod(home, 0o000)
    try:
        report = audit_gateway_home(home)
...
def test_the_gateway_home_audit_does_not_touch_the_tree(tmp_path):
    home = _build(tmp_path / "gateway", "SOUL.md", "abandoned.log", dirs=("memories", "sessions"))
    before = _snapshot(home)

    audit_gateway_home(home)

    assert _snapshot(home) == before


def test_the_default_gateway_home_is_the_one_mac_paths_resolves(clean_home, monkeypatch):
    # An un-migrated host still pins the gateway outside the root; the audit has
    # to follow the resolver there rather than assume $MAC_HOME/openclaw.
    monkeypatch.setenv("MAC_HOME", str(clean_home / "dot-mac"))
    elsewhere = _build(clean_home / "elsewhere", "SOUL.md")
    monkeypatch.setenv("HERMES_HOME", str(elsewhere))

    report = audit_gateway_home()

    assert report["home_path"] == str(elsewhere)
    assert report["by_classification"][CANONICAL] == ["SOUL.md"]

...
def test_stock_openclaw_artifacts_are_pinned_and_do_not_invoke_nemoclaw() -> None:
    ...
    assert 'OPENCLAW_IMAGE_REVISION="8"' in installer
...
def test_mac_continuity_plugin_registers_runtime_hook_and_tools() -> None:
    ...
    script = f"""
      ...
      if (!["mac_memory_recall", "mac_mood_current", "mac_mood_set", "mac_mood_clear", "mac_config_flag_list", "mac_config_flag_set", "mac_config_flag_clear", "curiosity_candidate_submit", "curiosity_candidates_list", "curiosity_abuse_frame"].every((name) => tools.has(name))) process.exit(3);
...
def test_verify_waits_for_new_sandbox_and_gateway_health(tmp_path: Path) -> None:
    ...
    openshell.write_text(
        ...
        "      *'plugins inspect mac-continuity'*) printf '%s\\n' '{\"plugin\": {\"imported\": true, \"status\": \"loaded\", \"toolNames\": [\"mac_memory_recall\", \"mac_mood_current\", \"mac_mood_set\", \"mac_mood_clear\", \"mac_config_flag_list\", \"mac_config_flag_set\", \"mac_config_flag_clear\", \"curiosity_candidate_submit\", \"curiosity_candidates_list\", \"curiosity_abuse_frame\"], \"hookNames\": [\"before_prompt_build\"]}}' ;;\n"

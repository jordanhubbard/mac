...
def test_classify_named_children_directory_and_symlink(tmp_path):
    from mac.hermes_home_audit import classify_named_children

    d = tmp_path / "c"
    d.mkdir()
    (d / "keep").mkdir()
    (d / "link").symlink_to(d / "keep")
    items = {item["name"]: item for item in classify_named_children(d, {"keep", "link"})}
    assert items["keep"]["kind"] == "directory"
    assert items["link"]["kind"] == "symlink"


def test_nonstandard_kind_oserror(tmp_path, monkeypatch):
    from mac import hermes_home_audit as hha

    home = make_home(tmp_path)
    ghost = home / "mystery.bin"
    ghost.write_text("abc", encoding="utf-8")
    real_is_symlink = Path.is_symlink
    ...
    monkeypatch.setattr(Path, "is_symlink", boom)
    results = {item["path"]: item for item in hha._collect_nonstandard(home)}
    assert results["mystery.bin"]["kind"] == "other"


def test_script_path_dedup_continue(tmp_path, monkeypatch):
    from mac import hermes_home_audit as hha

    home = make_home(tmp_path)
    scripts = home / "scripts"
    scripts.mkdir()
    target = scripts / "dup.py"
    target.write_text("print(1)\n", encoding="utf-8")
    real = hha.safe_iterdir
    ...
    monkeypatch.setattr(hha, "safe_iterdir", fake)
    monkeypatch.setattr(hha, "_safe_iterdir", fake)
    results = hha._collect_scripts(home)
    assert [item["path"] for item in results] == ["scripts/dup.py"]
